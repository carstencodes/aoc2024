...
type SearchStringRevised = char * char * char
type FoundChar = (int * int)
type Finding = FoundChar * FoundChar * FoundChar * FoundChar
type FindingRevised = FoundChar * FoundChar * FoundChar * FoundChar * FoundChar * FoundChar

type FoundChars = FoundChar array
type Findings = Finding list

type FindingsRevised = FindingRevised list
...
let isInDimRevised hayStack pos1 pos2 pos3 =
    let rowCount, colCount = dim hayStack
...
let fromHayStackRevised (hs: Matrix) p1 p2 p3 = 
    let r1, c1 = p1
    let r2, c2 = p2
    let r3, c3 = p3

    Some (hs.[r1].[c1], hs.[r2].[c2], hs.[r3].[c3])
...
let getCharsRevised (hayStack: Matrix) pos1 pos2 pos3 =
    let notLeavingHayStack= isInDimRevised hayStack pos1 pos2 pos3 
    // printfn "%A %b" (pos1, pos2, pos3, pos4) notLeavingHayStack

    if notLeavingHayStack
    then fromHayStackRevised hayStack pos1 pos2 pos3
    else None
...
let matchesRevised (straw: SearchStringRevised) (needle: SearchStringRevised) =
    let s1, s2, s3 = straw
    let n1, n2, n3 = needle

    ((s1 = n1) && (s2 = n2) && (s3 = n3)) || ((s1 = n3) && (s2 = n2) && (s3 = n1))
...
let searchDiagonalForwardUpAndForwardDown hayStack needle i j = 
    let pos1 = (i - 1, j + 1)
    let pos2 = (i, j)
    let pos3 = (i + 1, j - 1)
    ...
    let straw1 = getCharsRevised hayStack pos1 pos2 pos3
    let straw2 = getCharsRevised hayStack rpos1 rpos2 rpos3

    match (straw1, straw2) with
    | Some s, Some t -> if (matchesRevised s needle && matchesRevised t needle) then Some (pos1, pos2, pos3, rpos1, rpos2, rpos2) else None
    | _ -> None

let searchDiagonalForwardDownAndBackwardUp hayStack needle i j = 
    let pos1 = (i - 1, j + 1)
    let pos2 = (i, j)
    let pos3 = (i + 1, j - 1)
...
let searchDiagonalBackwardUpAndBackwardDown hayStack needle i j = 
    let pos1 = (i + 1, j + 1)
    let pos2 = (i, j)
    let pos3 = (i - 1, j - 1)
...
let searchDiagonalBackwardDownAndForwardUp hayStack needle i j = 
    let pos1 = (i + 1, j - 1)
    let pos2 = (i, j)
    let pos3 = (i - 1, j + 1)
...
let findX (hayStack: Matrix) (needle: SearchStringRevised) (dimensions: int * int) (i: int) (j: int) : FindingsRevised =
    let mutable findings = []
    
    findings <- (searchDiagonalForwardUpAndForwardDown hayStack needle i j) :: findings
    findings <- (searchDiagonalForwardDownAndBackwardUp hayStack needle i j) :: findings
    findings <- (searchDiagonalBackwardUpAndBackwardDown hayStack needle i j) :: findings
    findings <- (searchDiagonalBackwardDownAndForwardUp hayStack needle i j) :: findings

    findings |> List.choose id

let involvesAsX (hayStack: Matrix) (needle: SearchStringRevised) (dimensions: int * int) (i: int) (j: int) : FindingsRevised =
    if hayStack.[i].[j] = 'A'
    then findX hayStack needle dimensions i j
    else []
...
let mapInvolveRevised mapping rows columns =
    let mutable findings = []

    for i in rows do
        for j in columns do
            let localFindings: FindingsRevised = (mapping i j |> Seq.toList)
            findings <- findings |> List.append localFindings
...
let flattenFindingsRevised (findings: FindingsRevised): FoundChars =
    let mutable result = []

    for finding in findings do
        let a, b, c, d, e, f = finding
...
let applyFindingsToMapRevised m (f: FindingsRevised) =
    let rows, columns = dim m

    let allFindings = (flattenFindingsRevised f)
...
let transposeMasAsX m =
    ...
    let involvesMasAsX (i: int) (j: int) = involvesAsX m search (rowCount, columnCount) i j

    let involvedMasAsX = mapInvolveRevised involvesMasAsX rowIndices columnIndices

    let countXmas = (involvedMasAsX |> List.length) / 4

    countXmas, (applyFindingsToMapRevised m involvedMasAsX)
...
let count, transposed  = transposeMasAsX parsed
