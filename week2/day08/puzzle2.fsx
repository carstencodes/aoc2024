...
let rec findAntinodeForAntenna dimensions current next = 
    let difference = distance current next
    let antinode = addPoint next difference

    if staysInsideMap dimensions antinode
    then
        let nextPossibleAntinodes = findAntinodeForAntenna dimensions next antinode 
        match nextPossibleAntinodes with 
        | Some p -> Some (antinode::p)
        | None -> Some [antinode]
    else None
    ...
    for frequency in antennas.Keys do
        ...
        let findAntiNodesForAntenna index antenna = 
            ...
            antinodes <- antinodes |> Map.change frequency (fun q ->
                match q with 
                | Some s -> Some (s @ (antennaNodesLeft|> List.concat))
                | None -> None
            )
